...
Module startup is atomic: manifests and entrypoints are validated before registrations are committed. Duplicate registry keys, missing dependencies, and cycles fail startup. Workflow steps receive only original input and explicitly declared dependency results; optional failures become warnings while required failures skip dependents.
